...
function queryme($arguments)
{
	...
	if ( is_array($arguments) && !isset($_SERVER["argv"]) )
	...
	for ( $i=0; $i<count($args); $i++ )
	{
		...
		else if ( $arg=="-n2" || $arg=="--any" )			$mode = SPH_MATCH_ANY;
		...
		else if ( $arg=="-l2" || $arg=="--limit" )		$limit = (int)$args[++$i];
	...
	////////////////
	/*
	if ( $res["total"] != 0 &&is_array($res["matches"]) )
	{
		return $res["matches"];
	}
	else
		...
		*/
		if ( $res["total"] != 0 &&is_array($res["matches"]) )
		{
			...
			foreach ( $res["matches"] as $docinfo )
			{
				...
				array_push($docs, findit($docinfo['id'], $host));
			...
			$n--;
			$reply = $cl->BuildExcerpts($docs, 'test1', $q, array("single_passage" => false));
			/*
			for($i=0;$i<$n;$i++)
				print $reply[$i]."<br><br>";
			*/
